...
\begin{document}
    ...
    \subsection{Schlüsselerzeugung}
        ...
        \subsubsection{Das modulare Inverse}
            \enquote{Des weiteren [wird] eine zu $\Phi(n)$ teilerfremde Zufallszahl $e$ [gewählt] und [...] hierzu die \emph{modulare inverse} Zahl $d$ bezüglich $\Phi(n)$ [berechnet]. Für $d$ gilt:}\footcite[279]{beutelspacher2015}
            \begin{equation}
                d\cdot e\mod{\Phi(n)} = 1
            \end{equation}
            \enquote{$d$ kann berechnet werden mit dem erweiterten Euklidischen Algorithmus}\footcite[77]{ertel2003}
